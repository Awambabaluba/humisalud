...
export const Route = createFileRoute("/en/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Compare humidifiers with editorial criteria: m² calculator, real running cost and verified reviews. For babies, bedrooms, living rooms and allergies." },
  ...
  return (
    <section className="border-b border-border">
      <div className="mx-auto max-w-4xl px-5 pt-14 pb-12 sm:pt-20 sm:pb-16 text-center">
        <p className="text-xs uppercase tracking-[0.22em] text-primary font-medium">Buying guide · Updated June 2026</p>
        ...
        <p className="mx-auto mt-6 max-w-2xl border-y border-primary/25 py-4 font-display text-xl sm:text-2xl lg:text-3xl font-semibold text-foreground leading-[1.18]">
          Breathe better tonight: <span className="text-primary">end the dry throat, blocked nose and broken sleep.</span>
        </p>
        <p className="mx-auto mt-4 max-w-2xl text-base sm:text-lg text-muted-foreground leading-relaxed">
          We analysed <strong className="text-foreground font-medium">42 ultrasonic and evaporative humidifiers</strong> for the bedroom, baby, living room and allergies. We compare m² performance, dB noise, runtime, cleaning and real monthly running cost.
        </p>
        <div className="mt-7 flex flex-wrap justify-center gap-2 text-xs">
          {["For babies", "Bedroom", "Large living room", "Allergies & asthma", "Low consumption", "Silent"].map((t) => (
        ...
        <dl className="mx-auto mt-10 grid max-w-md grid-cols-3 gap-4 rounded-2xl border border-border bg-secondary/40 p-5 sm:p-6">
          ...
          <div>
            <dt className="text-[11px] uppercase tracking-wider text-muted-foreground">Sponsorships</dt>
...
function CategoriasSection() {
  ...
  return (
    <section className="mx-auto max-w-6xl px-5 py-12 sm:py-16">
      <SectionHeader eyebrow="By use" title="What will you use it for?" subtitle="Each situation weighs things differently. The baby's room asks for silence; the living room asks for output." />
  ...
  return (
    <section className="border-y border-border bg-secondary/30">
      <div className="mx-auto max-w-6xl px-5 py-16 sm:py-20">
        <SectionHeader eyebrow="2026 ranking" title="The four we would recommend today" subtitle="If you're in a hurry. If you want to understand why they're here, take a look at how we work." />
        <div className="mt-10 grid gap-5 md:grid-cols-2 lg:grid-cols-4">
          {productos.map((p, i) => (
            <ProductCard key={p.slug} producto={p} badge={i === 0 ? "Best pick" : i === 1 ? "Best value" : undefined} highlight={i === 0} />
          ))}
        </div>
        <div className="mt-8">
...
function SelectorSection() {
  return (
    <section className="mx-auto max-w-6xl px-5 py-16 sm:py-20">
      <SectionHeader eyebrow="Selector" title="Tell us about the room and we'll tell you which one" subtitle="Five questions. No endless forms." />
      <div className="mt-10">
        <Selector />
      </div>
    </section>
...
function MethodSection() {
  return (
    <section className="border-t border-border bg-secondary/30">
      <div className="mx-auto max-w-6xl px-5 py-16 sm:py-20">
        <SectionHeader eyebrow="How we work" title="Why this site doesn't look like the rest" subtitle="Analysis first, commissions after. Never the other way around." />
        <div className="mt-10">
          <MethodologyBlock />
        </div>
...
function FAQSection() {
  return (
    <section className="mx-auto max-w-3xl px-5 py-16 sm:py-20">
      <FAQ items={[
        { q: "Do you test the humidifiers in person?", a: "No. We say it without dressing it up: we work with official spec sheets and a lot of verified reviews. We think that's better than inventing tests we haven't done." },
        { q: "How do you choose the models in the ranking?", a: "We use a rubric with weights for m² performance, value, ease of cleaning, noise, runtime and spare parts. It's published in full in Methodology." },
        { q: "Why are some prices a range or shown as pending?", a: "Electronics prices change almost daily. When we can't guarantee the exact figure, we mark it as pending instead of putting up a number that will be a lie tomorrow." },
        { q: "What is the ideal indoor humidity?", a: "Between 40% and 60% (per Spanish RITE building code and Philips). Below 30% you'll feel dry skin and mucosa; above 65% mould starts to appear." },
        { q: "Can a humidifier be dangerous?", a: "If you don't clean it, yes. There are documented medical cases of humidifier lung. Distilled water, daily change and cleaning every two or three days avoids the problem." },
      ]} />
    </section>
  );
}
      ...
      <div className="relative overflow-hidden rounded-3xl border border-border bg-gradient-to-br from-primary/90 to-mist-deep p-8 sm:p-14 text-primary-foreground">
        ...
        <div className="relative max-w-2xl">
          ...
          <p className="mt-3 text-base text-primary-foreground/80">
            Instead of reading ten lists that look alike, tell us the room's square metres, your budget and what you want it for. We'll tell you what fits.
          </p>
          <div className="mt-7">
