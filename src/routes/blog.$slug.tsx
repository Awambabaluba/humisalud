...
export const Route = createFileRoute("/blog/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: post ? `/blog/${post.slug}` : "/blog" },
      ],
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-2xl px-5 py-20 text-center">
      ...
      <Link to="/blog" className="mt-4 inline-block text-primary underline">Ver el blog</Link>
    </div>
  ),
...
function BlogPostPage() {
  ...
  return (
    <article className="mx-auto max-w-3xl px-5 py-12 sm:py-16 prose-editorial">
      <Breadcrumbs items={[{ to: "/", label: "Inicio" }, { to: "/blog", label: "Blog" }, { label: post.titulo }]} />
      <header className="mt-6 not-prose">
        <p className="text-xs uppercase tracking-[0.18em] text-primary font-medium">{post.categoria}</p>
        <h1 className="mt-3 font-display text-4xl sm:text-5xl font-semibold tracking-tight">{post.titulo}</h1>
        ...
        <section className="mt-14 not-prose">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2">
            {otros.map((p) => (
              ...
              >
                <p className="text-xs uppercase tracking-wider text-primary font-medium">{p.categoria}</p>
