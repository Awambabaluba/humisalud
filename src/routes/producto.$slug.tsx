...
export const Route = createFileRoute("/producto/$slug")({
  ...
  head: ({ loaderData }) => {
    ...
    return {
      meta: [
        ...
        { property: "og:url", content: p ? `/producto/${p.slug}` : "/" },
      ],
      links: p ? [{ rel: "canonical", href: `/producto/${p.slug}` }] : [],
      scripts: p ? [{
        ...
        children: JSON.stringify({
          ...
          review: {
            ...
            reviewRating: { "@type": "Rating", ratingValue: p.valoracionEditorial, bestRating: 100 },
          },
        }),
      }] : [],
    };
  ...
  notFoundComponent: () => (
    <div className="mx-auto max-w-2xl px-5 py-20 text-center">
      ...
      <Link to="/mejores-humidificadores" className="mt-4 inline-block text-primary underline">Ver el ranking</Link>
    </div>
  ),
    ...
    <article className="mx-auto max-w-4xl px-5 py-12 sm:py-16">
      ...
      <header className="mt-6 grid gap-8 md:grid-cols-[1.4fr_1fr] items-start">
        <div>
          ...
          <h1 className="mt-2 font-display text-4xl sm:text-5xl font-semibold tracking-tight">{p.nombre}</h1>
          ...
          <div className="mt-6 inline-flex items-center gap-3 rounded-2xl border border-border bg-card px-5 py-4">
            <div>
              <p className="text-xs uppercase tracking-wider text-muted-foreground">Valoración editorial</p>
              <p className="font-display text-3xl font-semibold">{p.valoracionEditorial}<span className="text-base text-muted-foreground">/100</span></p>
            </div>
          ...
          <p className="mt-3 text-xs text-muted-foreground inline-flex items-center gap-1.5">
            <CalendarDays className="h-3.5 w-3.5" /> Actualizado el {new Date(p.fechaActualizacion).toLocaleDateString("es-ES", { day: "numeric", month: "long", year: "numeric" })}
          </p>
        </div>
        ...
        <aside className="rounded-3xl border border-border bg-card p-6 shadow-soft">
          ...
          <p className="mt-4 text-xs text-muted-foreground">Tecnología: <strong className="text-foreground font-medium">{p.tecnologia}</strong></p>
          {PRODUCT_IMAGE_CREDITS[p.slug] && (
            <p className="mt-3 text-[11px] text-muted-foreground">Imagen: cortesía de {PRODUCT_IMAGE_CREDITS[p.slug]}.</p>
          )}
        </aside>
      </header>
      ...
      <section className="mt-12">
        ...
        <div className="mt-5"><ProsCons ventajas={p.ventajas} inconvenientes={p.inconvenientes} /></div>
      </section>
      ...
      <section className="mt-14">
        ...
        <div className="mt-5 grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {alternativas.map((a) => <ProductCard key={a.slug} producto={a} />)}
        </div>
      </section>
      ...
      <section className="mt-14 rounded-3xl border border-border bg-secondary/40 p-6 sm:p-8">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Este humidificador no lo hemos tenido en casa. La valoración sale de la ficha oficial, del precio que
          hemos visto y de cruzar reseñas verificadas. Si quieres saber cómo lo puntuamos exactamente, mira nuestra{" "}
          <Link to="/metodologia" className="underline underline-offset-4">página de metodología</Link>.
        </p>
        <div className="mt-5">
          <AffiliateButton
            href={p.enlaceAfiliado}
      ...
      <section className="mt-14">
        <FAQ items={[
          { q: "¿Lo habéis probado en casa?", a: "No. Lo analizamos con la ficha técnica oficial y las reseñas verificadas de quien sí lo ha comprado." },
          { q: "¿El precio que ponéis es el definitivo?", a: "Es lo que vimos en la tienda el día de la última actualización. En electrónica los precios bailan, así que compruébalo antes de pasar por caja." },
          { q: "¿Por qué este y no otro?", a: "Porque al pasarlo por nuestra rúbrica (rendimiento, calidad-precio, limpieza, ruido) sale por delante de las alternativas comparables de su gama." },
        ]} />
      </section>
    </article>
  );
  ...
  return (
    <div className="rounded-xl bg-secondary/60 px-3 py-2.5">
      <div className="text-[11px] uppercase tracking-wider text-muted-foreground flex items-center gap-1.5">
        <span className="text-primary [&_svg]:h-3.5 [&_svg]:w-3.5">{icon}</span>{label}
      </div>
...
function Block({ title, items, tone }: { title: string; items: string[]; tone: "positive" | "negative" }) {
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <h3 className={`font-display text-base font-semibold ${tone === "positive" ? "text-foreground" : "text-foreground"}`}>{title}</h3>
